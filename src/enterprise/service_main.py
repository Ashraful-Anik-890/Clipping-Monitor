"""
Windows Service for Enterprise Monitoring Agent - COMPLETE IMPLEMENTATION

This is the ONLY file needed to install, run, and manage the Windows service.
All functionality has been merged into this single file for simplicity.
...
sys.path.insert(0, str(project_root / "enterprise"))
...
class MonitoringEngine:
    ...
    def on_keystroke_event(self, event):
        ...
        try:
            logger.debug(f"Keystroke logged: {event.get('application', 'unknown')}")
        except Exception as e:
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        try:
            ...
            logger.critical(f"Failed to start service dispatcher: {e}", exc_info=True)
            print(f"ERROR: Failed to start service dispatcher: {e}")
            print("This should only be called by Windows Service Control Manager")
            sys.exit(1)
    else:
        ...
        if command == 'install':
            ...
            try:
                ...
                print("\nMonitoring engine started. Monitoring activity...\n")
                while True:
                    time.sleep(1)
